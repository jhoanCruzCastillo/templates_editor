...
// Editor de columnas para subtipo "jerarquica" — misma interfaz que MatrizPeriodosEditor
// (arrastrar para reordenar, nombre editable inline, engranaje para posición en Excel +
// agrupación bajo cabecera, eliminar), pero sin el botón de columna dinámica: en su lugar,
// un botón para alternar el nivel jerárquico (padre/hijo) de cada columna. La interacción de
// agregar items hijos de la jerarquía (horizontal) vive en ExampleTableEditor y no se toca aquí.
export default function JerarquicaColumnsEditor({ config, onChange }: Props) {
  ...
  const renderColumn = (col: ColumnaTabla, rowSpan: number) => (
    ...
    >
      <div className="flex items-center gap-1">
        ...
        <button
          onClick={() => toggleNivel(col.id)}
          title={col.nivel === 'padre' ? 'Nivel: Padre (clic para cambiar a Hijo)' : 'Nivel: Hijo (clic para cambiar a Padre)'}
          className={`text-[9px] font-semibold shrink-0 px-1 py-0.5 rounded transition-colors ${
            col.nivel === 'padre' ? 'text-amber-600 hover:bg-amber-50' : 'text-blue-600 hover:bg-blue-50'
          }`}
        >
